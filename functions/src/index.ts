...
type BankQuestionRecord = { question?: string; publicationId?: string; publicationTitle?: string };
...
export const repairQuestionBanks = onCall({ region: "us-central1", timeoutSeconds: 60, memory: "512MiB", maxInstances: 1, concurrency: 1, enforceAppCheck: false }, async request => {
  if (!request.auth) throw new HttpsError("unauthenticated", "Sign in to repair question banks."); const userId = request.auth.uid;
  const [publicationSnapshot, questionSnapshot] = await Promise.all([db.collection(`users/${userId}/publications`).get(), db.collection(`users/${userId}/questions`).get()]); const known = new Set(publicationSnapshot.docs.map(document => document.id)); const orphaned = new Map<string, { title: string; count: number }>();
  questionSnapshot.docs.forEach(document => { const question = document.data() as BankQuestionRecord; const publicationId = String(question.publicationId ?? ""); if (!publicationId || known.has(publicationId)) return; const prior = orphaned.get(publicationId); orphaned.set(publicationId, { title: String(question.publicationTitle ?? prior?.title ?? "Recovered question bank"), count: (prior?.count ?? 0) + 1 }); });
  const entries = [...orphaned.entries()]; for (let start = 0; start < entries.length; start += 400) { const batch = db.batch(); entries.slice(start, start + 400).forEach(([id, bank]) => batch.set(db.doc(`users/${userId}/publications/${id}`), { title: bank.title, filename: "recovered-question-bank", category: "imported", imported: true, enabled: true, dailyQuiz: true, dailyLesson: true, priority: "normal", sensitivity: "private", externalAiApproved: false, status: "ready", storagePath: "", fileBytes: 0, questionCount: bank.count, recoveredAt: FieldValue.serverTimestamp() })); await batch.commit(); }
  return { restoredBanks: entries.length, restoredQuestions: entries.reduce((total, [, bank]) => total + bank.count, 0), totalQuestions: questionSnapshot.size };
});

export const mergeImportedQuestionBanks = onCall({ region: "us-central1", timeoutSeconds: 60, memory: "512MiB", maxInstances: 1, concurrency: 1, enforceAppCheck: false }, async request => {
  if (!request.auth) throw new HttpsError("unauthenticated", "Sign in to combine question banks."); const userId = request.auth.uid; const sourceId = String(request.data?.sourceId ?? ""); const targetId = String(request.data?.targetId ?? ""); if (!sourceId || !targetId || sourceId === targetId) throw new HttpsError("invalid-argument", "Choose two different imported question banks.");
  const [sourceSnapshot, targetSnapshot] = await Promise.all([db.doc(`users/${userId}/publications/${sourceId}`).get(), db.doc(`users/${userId}/publications/${targetId}`).get()]); if (!sourceSnapshot.exists || !targetSnapshot.exists || !sourceSnapshot.data()?.imported || !targetSnapshot.data()?.imported) throw new HttpsError("failed-precondition", "Only imported question banks can be combined."); const targetTitle = String(targetSnapshot.data()?.title ?? "Imported question bank");
  const [from, into] = await Promise.all([existingPublicationQuestions(userId, sourceId), existingPublicationQuestions(userId, targetId)]); if (from.size > 450) throw new HttpsError("failed-precondition", "This bank is too large to combine safely."); const seen = new Set(into.docs.map(document => normalizeQuestion(String((document.data() as BankQuestionRecord).question ?? "")))); const transfers: typeof from.docs = []; const duplicates: typeof from.docs = []; from.docs.forEach(document => { const fingerprint = normalizeQuestion(String((document.data() as BankQuestionRecord).question ?? "")); if (seen.has(fingerprint)) duplicates.push(document); else { seen.add(fingerprint); transfers.push(document); } }); const batch = db.batch(); transfers.forEach(document => batch.update(document.ref, { publicationId: targetId, publicationTitle: targetTitle, movedAt: FieldValue.serverTimestamp() })); duplicates.forEach(document => batch.delete(document.ref)); batch.set(targetSnapshot.ref, { questionCount: into.size + transfers.length, updatedAt: FieldValue.serverTimestamp() }, { merge: true }); batch.delete(sourceSnapshot.ref); await batch.commit(); return { combined: transfers.length, duplicatesRemoved: duplicates.length, questionCount: into.size + transfers.length };
});

export const removeExactQuestionDuplicates = onCall({ region: "us-central1", timeoutSeconds: 60, memory: "512MiB", maxInstances: 1, concurrency: 1, enforceAppCheck: false }, async request => {
  if (!request.auth) throw new HttpsError("unauthenticated", "Sign in to remove duplicate questions."); const userId = request.auth.uid; const publicationId = String(request.data?.publicationId ?? ""); if (!publicationId) throw new HttpsError("invalid-argument", "publicationId is required."); const publicationRef = db.doc(`users/${userId}/publications/${publicationId}`); if (!(await publicationRef.get()).exists) throw new HttpsError("not-found", "Question bank not found."); const snapshot = await existingPublicationQuestions(userId, publicationId); const seen = new Set<string>(); const duplicates = snapshot.docs.filter(document => { const fingerprint = normalizeQuestion(String((document.data() as BankQuestionRecord).question ?? "")); if (!fingerprint || !seen.has(fingerprint)) { seen.add(fingerprint); return false; } return true; }); for (let start = 0; start < duplicates.length; start += 400) { const batch = db.batch(); duplicates.slice(start, start + 400).forEach(document => batch.delete(document.ref)); await batch.commit(); } const remaining = snapshot.size - duplicates.length; await publicationRef.set({ questionCount: remaining, duplicateCleanupAt: FieldValue.serverTimestamp() }, { merge: true }); return { removed: duplicates.length, remaining };
});
