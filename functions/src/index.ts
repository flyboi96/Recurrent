...
const WHOLE_DOCUMENT_OUTPUT_TOKENS = 12_000;
...
async function generateWholeDocumentSet(userId: string, publicationId: string, publication: StoredPublication) {
  ...
  const prompt = `Create a comprehensive, coverage-driven recurrent-study bank from this entire aircraft operating handbook. Do not select an arbitrary total question count. First identify every meaningful operational learning objective in the handbook: procedures, systems, limitations, warnings/cautions, emergency actions, decision conditions, failure indications, and exact operational values. Exclude document-navigation trivia such as title, revision history, volume, table of contents, page locations, and document organization.

Return JSON only in this exact shape: {"objectives":[{"id":"stable-kebab-id","objective":"what the learner must understand or do","category":"systems|procedures|limitations|emergency|operations","startPage":1,"endPage":1,"importance":"critical|high|standard","requiredQuestions":1}],"questions":[{"objectiveId":"stable-kebab-id","question":"","answer":"","explanation":"why this matters operationally","category":"","page":1,"strictRecall":false}]}. Use one question for each standard objective, two for high objectives, and up to three for critical objectives where distinct recall is needed. Generate questions for as many objectives as fit; any uncovered objectives will be completed later from their indexed source passages without rereading this full PDF. Every question must be answerable from the PDF, use the most relevant page, and test operational understanding rather than document trivia.`;
  ...
  const text = payload.candidates?.[0]?.content?.parts?.map(part => part.text ?? "").join("") ?? ""; let plan: WholeDocumentPlan; try { plan = parseJson<WholeDocumentPlan>(text); } catch { throw new HttpsError("unavailable", "Gemini returned an incomplete coverage plan. Please retry."); }
  const objectives = (plan.objectives ?? []).filter(objective => objective.id?.trim() && objective.objective?.trim() && Number.isInteger(objective.startPage) && Number.isInteger(objective.endPage) && objective.requiredQuestions > 0).slice(0, 180);
  const validObjectiveIds = new Set(objectives.map(objective => objective.id)); const accepted = (plan.questions ?? []).filter(question => question.question?.trim() && question.answer?.trim() && question.explanation?.trim() && Number.isInteger(question.page) && validObjectiveIds.has(question.objectiveId)); if (objectives.length < 10 || accepted.length < 10) throw new HttpsError("unavailable", "Gemini returned too little operational coverage. Please retry.");
  await recordSuccessfulUsage(userId, Number(payload.usageMetadata?.promptTokenCount ?? pages * WHOLE_DOCUMENT_TOKENS_PER_PAGE), Number(payload.usageMetadata?.candidatesTokenCount ?? estimateTokens(text)));
  ...
  const target = objectives.reduce((total, objective) => total + Math.min(3, Number(objective.requiredQuestions)), 0); const complete = accepted.length >= target;
  batch.set(db.doc(`users/${userId}/publications/${publicationId}`), { status: "ready", wholeDocumentVersion: WHOLE_DOCUMENT_VERSION, coverageObjectives: objectives, coverageObjectiveCount: objectives.length, coverageQuestionTarget: target, coverageQuestionCount: accepted.length, coverageComplete: complete, questionCount: accepted.length, wholeDocumentGeneratedAt: FieldValue.serverTimestamp() }, { merge: true }); await batch.commit(); return { generated: accepted.length, complete, nextPage: null, remainingObjectives: complete ? 0 : objectives.length };
}
...
async function generateRemainingCoverage(userId: string, publicationId: string, publication: StoredPublication) {
  ...
  const response = await fetch(geminiUrl(process.env.GEMINI_MODEL || "gemini-3.6-flash", apiKey), { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ contents: [{ parts: [{ text: `${prompt}\n\n${source}` }] }], generationConfig: { responseMimeType: "application/json", maxOutputTokens: OBJECTIVE_BATCH_OUTPUT_TOKENS, temperature: 0.15 } }) }); if (!response.ok) throw new HttpsError("unavailable", `Gemini coverage request failed (${response.status}).`);
  const payload = await response.json() as { candidates?: { content?: { parts?: { text?: string }[] } }[]; usageMetadata?: { promptTokenCount?: number; candidatesTokenCount?: number } }; const text = payload.candidates?.[0]?.content?.parts?.map(part => part.text ?? "").join("") ?? ""; let generated: WholeDocumentQuestion[]; try { generated = parseJson<{ questions: WholeDocumentQuestion[] }>(text).questions; } catch { throw new HttpsError("unavailable", "Gemini returned an incomplete coverage batch. Please retry."); }
  const allowed = new Set(selected.map(objective => objective.id)); const accepted = generated.filter(question => allowed.has(question.objectiveId) && question.question?.trim() && question.answer?.trim() && question.explanation?.trim() && Number.isInteger(question.page)); if (!accepted.length) throw new HttpsError("unavailable", "Gemini returned no usable coverage questions. Please retry."); await recordSuccessfulUsage(userId, Number(payload.usageMetadata?.promptTokenCount ?? estimateTokens(source)), Number(payload.usageMetadata?.candidatesTokenCount ?? estimateTokens(text)));
