...
Return a compact JSON map with at most 80 objectives. Combine closely related standard details under one objective; do not omit critical or high operational material. Use one question for each standard objective, two for high objectives, and up to three for critical objectives where distinct recall is needed. This request creates the coverage map only; source-grounded questions will be generated in smaller follow-up batches.`;
  ...
  const text = payload.candidates?.[0]?.content?.parts?.map(part => part.text ?? "").join("") ?? ""; await recordSuccessfulUsage(userId, Number(payload.usageMetadata?.promptTokenCount ?? pages * WHOLE_DOCUMENT_TOKENS_PER_PAGE), Number(payload.usageMetadata?.candidatesTokenCount ?? estimateTokens(text))); let plan: WholeDocumentPlan; try { plan = parseJson<WholeDocumentPlan>(text); } catch { throw new HttpsError("unavailable", "Gemini returned an incomplete coverage plan. Please retry."); }
